...
**Last Updated**: August 10, 2026 — Save Point "ED04"
...
- `neon-sphere.tsx` is an original GLSL shader — do not swap it for a copied third-party sphere. Texture-mapped from `/images/sphere-ref.png` (equirectangular UV); the mapping is degenerate at the poles, so the pole-cap blend must stay wide enough (`smoothstep(0.82,0.97,...)`) with its own finer `capDetail` noise octave, or it reads as a smeared starburst/bald patch whenever a pole rotates into view — never shrink the cap blend or drop `capDetail` without re-checking pole rotation. Use `NormalBlending`, not additive: additive pushes the rim/filament/specular terms past 1.0 and clips the ball to flat white. Specular is normal-based (`lightDir`/`ndl`/`pow(ndl,110.0)`), tracking the pointer — do not revert to a screen-space falloff, which leaves a fixed hazy bloom at the pointer's default `(0,0)`. Fade-in is owned solely by the wrapper's CSS `opacity` transition — do not reintroduce a shader-side `uFade`.
